...
// --- GLOBAL VARIABLES (Parent Definitions) ---
// これらは親ファイルで一度だけ定義し、子ファイルからは参照のみ行う
...
window.onload = function() {
    ...
    // Initialize Game Logic (if loaded)
    // ゲーム側の初期化関数をここで呼ぶ
...
window.onAuthDataLoaded = function(json) {
    ...
    if (foundUser) {
        ...
        // Render MyPage if active (Game logic might override renderMyPage)
        if(document.getElementById('view-mypage').classList.contains('active')) {
            if (typeof renderMyPage === 'function') renderMyPage();
        }
        
        // Refresh coins if game logic is present
        if (typeof fetchCoinData === 'function') fetchCoinData();
...
function updateUserDisplay() {
    ...
    if (currentUser) {
        ...
        // Game logic integration check
        let coins = 0;
        if (typeof getUserTotalCoins === 'function') {
            coins = getUserTotalCoins(name);
        } else {
            // Fallback calculation if game logic not loaded yet
            const pStats = playerStatsGlobal[name] || { wins: 0, games: 0 };
            coins = (pStats.wins * 50) + (pStats.games * 10);
        }
...
function updateSideMenuActiveState() {
    ...
    if(currentView === 'mypage') {
        if(items[2]) items[2].classList.add('active'); // My Page
    } else if (currentView === 'game') {
        if(items[1]) items[1].classList.add('active'); // Game
    } else {
        if(items[0]) items[0].classList.add('active'); // Tennis
    }
}
...
function selectAppMode(mode) {
    ...
    if (mode === 'tennis') {
        ...
        if(subTitle) subTitle.textContent = "DEEP SPACE INTEGRATED SYSTEM";
    } else if (mode === 'game') {
        switchMainView('game');
        if(bottomNav) bottomNav.classList.add('hidden');
        const title = document.getElementById('pageTitle');
        if(title) title.textContent = "GAME ARCHIVE";
        if(subTitle) subTitle.textContent = "TACTICAL SIMULATION MODULES";
    }
}
...
function switchMainView(viewName) {
    ...
    if(navs.length > 0) {
        if(viewName === 'top' && navs[0]) navs[0].classList.add('active');
        if(viewName === 'match' && navs[1]) navs[1].classList.add('active');
        if(viewName === 'dashboard' && navs[2]) navs[2].classList.add('active');
        if(viewName === 'ranking' && navs[3]) navs[3].classList.add('active');
        if(viewName === 'schedule' && navs[4]) navs[4].classList.add('active');
    }
...
function fetchWeather() {
    ...
    .catch(err => {
        ...
        if(el) el.innerHTML = '<div style="color:#666; font-size:0.8rem;">WEATHER SIGNAL LOST</div>';
    });
}
...
// ================== MAP LOGIC ==================
const courtDatabase = {
    "MOGUSA": { name: "百草テニスガーデン", address: "〒191-0033 東京都日野市百草878−1", tel: "042-591-1212", hours: "9:00～18:00", costStr: "¥4,500/h", costVal: 4500 },
    "NERIMA": { name: "練馬ステップイン", address: "〒177-0035 東京都練馬区南田中1丁目15−22", tel: "03-3904-7051", hours: "9:00～21:30", costStr: "¥4,000/h", costVal: 4000 }
};
...
function renderMapInfo(val) {
    ...
    if (!data) {
        c.innerHTML = `<div class="empty-map-state"><div class="spinning" style="display:inline-block; margin-bottom:10px;">❖</div><br>WAITING FOR NAVI INPUT...</div>`;
        return;
    }
    ...
    c.innerHTML = `
        <div class="info-card" style="margin-top:20px;">
            ...
            <div class="info-row">
                ...
                <div class="info-val">${displayAddress}<button onclick="copyToClipboard('${data.address}')" style="background:rgba(0,255,255,0.1); border:1px solid #00ffff; color:#00ffff; border-radius:4px; cursor:pointer; margin-left:10px; padding:2px 6px; font-size:0.7rem;">📋</button></div>
            </div>
...
function fetchScheduleData() {
    ...
    fetch(SCHEDULE_GAS_URL)
      ...
      .then(data => {
        ...
        if(data.length === 0) {
            list.innerHTML = '<div style="padding:20px;text-align:center;color:#888;">予定がありません</div>';
            return;
        }
      ...
      .catch(error => {
        console.error(error);
        list.innerHTML = '<div style="padding:20px;text-align:center;color:#888;">データ読み込みエラー<br>GAS URLを確認してください</div>';
      });
}
...
function renderScheduleList(targetMonth) {
    ...
    filtered.forEach((item, index) => {
        ...
        if (maxOk > 0 && item.counts.ok === maxOk) {
            ...
            crownHtml = '<span class="crown-icon">👑</span>';
        }
        ...
        li.innerHTML = `
            ...
            <div class="schedule-counts">
                <span class="count-ok"><span class="count-symbol">◯</span>${item.counts.ok}</span>
                <span class="count-maybe"><span class="count-symbol">△</span>${item.counts.maybe}</span>
                <span class="count-ng"><span class="count-symbol">✕</span>${item.counts.ng}</span>
            </div>
        `;
        list.appendChild(li);
    });
...
function showScheduleDetail(itemData) {
    ...
    if (!itemData.details || itemData.details.length === 0) {
        list.innerHTML = '<li style="padding:20px; text-align:center; color:#888;">回答データなし</li>';
    } else {
        ...
        sortedDetails.forEach(member => {
            ...
            let statusIcon = '';
            if(member.status === 'ok') statusIcon = '<span class="status-ok">◯</span>';
            else if(member.status === 'maybe') statusIcon = '<span class="status-maybe">△</span>';
            else statusIcon = '<span class="status-ng">✕</span>';
...
function processAnalysisData() {
    ...
    // Update User Display in Header (also triggers game logic coin update if loaded)
    updateUserDisplay();
...
function showModal(name) {
    ...
    radarChartInstance = new Chart(ctxR, {
        type: 'radar',
        data: { labels: ['勝率', '勝利', 'MVP', '試合数', 'ゲーム'], datasets: [{ label: '能力値', data: [ p.wins + p.losses > 0 ? (p.wins/(p.wins+p.losses)*100) : 0, safeCalc(p.wins, max.wins), safeCalc(p.mvpCount, max.mvpCount), safeCalc(p.matches, max.matches), safeCalc(p.games, max.games) ], backgroundColor:'rgba(0,255,255,0.2)', borderColor:'#00ffff', borderWidth:2 }] },
