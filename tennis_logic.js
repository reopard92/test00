...
window.onload = function() {
    ...
    // Game initialization (if game_logic.js is loaded)
...
function updateSideMenuActiveState() {
    ...
    if(currentView === 'mypage') {
        ...
        items[0].classList.add('active'); // Tennis
    }
}
...
function switchMainView(viewName) {
    ...
    if(titles[viewName]) document.getElementById('pageTitle').textContent = titles[viewName];
...
// ================== MAP LOGIC ==================
const courtDatabase = {
    "MOGUSA": { name: "百草テニスガーデン", address: "〒191-0033 東京都日野市百草878−1", tel: "042-591-1212", hours: "9:00～18:00", costStr: "¥4,500/h", costVal: 4500, mapSrc: "https://googleusercontent.com/maps.google.com/5" },
    "NERIMA": { name: "練馬ステップイン", address: "〒177-0035 東京都練馬区南田中1丁目15−22", tel: "03-3904-7051", hours: "9:00～21:30", costStr: "¥4,000/h", costVal: 4000, mapSrc: "" }
};
...
function renderAnalysisChart(players) {
    ...
    new Chart(ctx, {
        type: 'bar',
        data: { labels: tops.map(p=>p.name), datasets: [{ label:'勝率 (%)', data: tops.map(p=>(p.wins+p.losses)>0?p.wins/(p.wins+p.losses)*100:0), backgroundColor:'rgba(255,255,255,0.1)', borderColor:'#fff', borderWidth:1 }] },
